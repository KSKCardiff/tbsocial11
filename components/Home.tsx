  ...
  return (
    <div className="flex items-center justify-between py-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Social Buzz Scout – Repost</h1>
      </div>
      ...
      {data && (
        <div className="mt-6 grid md:grid-cols-5 gap-6">
          <div className="md:col-span-3 space-y-4">
            ...
            <section className="bg-neutral-900 border border-neutral-800 rounded-2xl p-4">
              <h3 className="text-lg font-medium mb-2">Seçilen Gönderi</h3>
              {data.winner ? (
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-neutral-500">Skor (oransal): {(data.winner.ratio || 0).toFixed(2)} • Mutlak: {data.winner.absoluteScore}</div>
                </div>
              ) : <div>Sonuç yok.</div>}
            </section>
              ...
              <div className="space-y-3 max-h-[360px] overflow-auto pr-2">
                {(data.posts || []).slice(1, 20).map((p: any) => (
                  <a key={p.id} href={p.url} target="_blank" className="block p-3 rounded-xl bg-neutral-950/60 border border-neutral-800 hover:border-neutral-700">
                    <div className="text-sm text-neutral-400 mb-1">@{p.author} • {p.platform} • r={p.ratio?.toFixed(2)}</div>
